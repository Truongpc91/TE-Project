(function($) {
    ...
    HT.createVariantRow = (attributeItem, variantItem) => {
        ...
        $row.append($('<td>').addClass('td-quantity').text('-'))
            ...
            .append($('<td>').addClass('td-sku').text(mainSku + '_' + classModified))
            .append($td)
        return $row
    }
            ...
            html = html + '<td colspan="6">'
                html = html + '<div class="updateVariant ibox">'
                    ...
                    html = html + '<div class="ibox-content">'
                        ...
                        // html = html + '</div>'
                        html = html + '<div class="image-variant">'
                        html = html + '<label for="file-upload">'
                        html = html +  '<div class="icon" style="cursor: pointer;">'
                        html = html + '<a type="button" class="upload-variant-picture">'
                        ...
                        html = html +'</div>'
                        html = html + '</label>'
                        html = html + '<input id="file-upload" type="file" style="display:none" name="variant_album"/>'
                        html = html + '<div class="small-text">Sử dụng nút chọn hình để thêm mới hình ảnh</div>'
                        html = html +'</div>'
        ...
        $(document).on('click', '.saveUpdateVariant', function(){
            ...
            let variant = {
                ...
                'album' : $("input[name='variant_album']").map(function(){
                    ...
                    const result = $(this).val().split(replace).join(replaceWith);
                    return result  
                }).get(),
            }
