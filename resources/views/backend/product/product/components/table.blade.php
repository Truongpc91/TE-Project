a<table class="table table-striped table-bordered">
    <thead>
        <tr>
            <th style="width:50px;">
            ...
            @foreach ($products as $product)
                <tr id="{{ $product->id }}">
                                ...
                                <div class="catalogue">
                                    ...
                                    @foreach ($product->product_catalogues as $val)
                                        @foreach ($val->product_catalogue_language as $cat)
                                            <a href="{{ route('admin.product.index', ['product_catalogue_id' => $val->id]) }}"
                                                title="">{{ $cat->name }}</a>
                                        @endforeach
                                    @endforeach
                                </div>
                    ...
                    <td class="text-center">
                        ...
                        <a href="{{ route('admin.product.delete', $product) }}" class="btn btn-danger"><i
