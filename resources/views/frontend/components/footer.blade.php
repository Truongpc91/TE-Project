        ...
        <div class="footer-upper">
            <div class="uk-grid uk-grid-medium">
                ...
                <div class="uk-width-large-3-5">
                    @if (count($menus['menu-footer']))
                        <div class="footer-menu">
                            <div class="uk-grid uk-grid-medium">
                                @foreach ($menus['menu-footer'] as $key => $val)
                                    @php
                                        ...
                                        <div class="ft-menu">
                                            ...
                                            <ul class="uk-list uk-clearfix">
                                                @foreach ($val['children'] as $children)
                                                    @php
                                                        $childrenName = $children['item']->languages->first()->pivot->name;
                                                        $childrenCanonical = write_url($children['item']->languages->first()->pivot->canonical);
                                                    @endphp
                                                <li><a href="{{ $childrenCanonical }}">{{ $childrenName }}</a></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    </div>
